...
textモードでは、選択範囲内にある`<img>`要素の絶対URLも付随情報として自動的に収集する（既定で最大5件、重複除去）。この画像URLは表示・RSS配信専用であり、比較用の値（comparisonValue）には一切含めない。画像だけが変化しても変更検知・通知は発生しない。RSS descriptionおよび管理画面の変更履歴では、収集した画像を元ページへのリンク付きの`<img>`として表示する。
...
- descriptionの内容はMonitorごとの`change_display_mode`設定に従う。既定の`both`では、変更されたラベル、旧値、新値の両方を含める（一覧（配列）モードのSelectionでは追加分を`Added:`、削除分を`Removed:`として示す）。`new_only`では新しい値だけを示す。一覧（配列）モードのSelectionでは新規追加分の項目のみを`Added:`のprefixなしで示し（削除分は示さない）、単一値（scalar）モードのSelectionでは新しい値をそのまま示す（旧値との差分表示は行わない）。単一値（scalar）モードのSelectionが`both`で変更差分を示す場合、旧値全体・新値全体を並べるのではなく、値を行単位（改行を含まない値は文単位）で差分比較し、実際に変わった行だけを示す。変更のあった行の前後5行までは文脈として未変更のまま表示し、それより離れた未変更行は`… (N unchanged lines)`へまとめて省略する。削除された行と追加された行が明らかに対応する場合は1行にまとめ、その行の中で実際に変わった語だけを、旧側は打消し線付きの赤、新側は太字の緑でハイライトする（管理画面の変更履歴でも同じ表示を使う）。日本語のように空白で語が区切られない文字列でも語単位で比較すること。ページが文言を変えずに改行位置だけを変えた場合は行同士がほとんど対応しなくなるため、その場合は値全体を語単位で比較し直す。語単位の比較では空白・改行の種類の違いを差分として扱わず（比較用の値が改行を空白へ畳むのと同じ理由）、未変更部分は新しい値の改行で表示する。値が大きすぎて行単位の差分が許容計算量を超える場合に限り、共通する前後の文脈だけを残す粗い差分表示へ退避する。一覧（配列）モードの`Added:`/`Removed:`表示は項目単位の差分であり、この行単位・語単位ハイライトの対象外とする。
- linkは元ページへ向ける。
...
- 単一値（scalar）Selectionの行単位・語単位差分（離れた2箇所の変更が1つの塊にまとめられないこと、変更行から離れた未変更行の省略、日本語の語単位ハイライト、計算量上限を超えた場合の粗い差分への退避）
