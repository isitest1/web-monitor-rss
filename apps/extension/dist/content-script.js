...
(() => {
  ...
  // ../../packages/selector-engine/src/token-quality.ts
  var UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  var LONG_HEX_PATTERN = /^[0-9a-f]{8,}$/i;
  var LONG_NUMERIC_PATTERN = /^\d{6,}$/;
  var FRAMEWORK_GENERATED_PATTERN = /^(css|sc|v|ember|mui|chakra|jsx|styled|emotion)-?[0-9a-z]{3,}$/i;
  var TRAILING_HASH_SUFFIX_PATTERN = /-([0-9a-z]{6,})$/i;
  ...
  // ../../packages/selector-engine/src/dom-utils.ts
  ...
  function checkCandidate(root, selector, target) {
    let matches;
    try {
  ...
  var DATA_ATTR_PRIORITY = ["data-testid", "data-test", "data-qa"];
  function stableDataAttributes(element) {
    ...
    for (const attr of Array.from(element.attributes)) {
      if (attr.name.startsWith("data-") && !DATA_ATTR_PRIORITY.includes(attr.name) && attr.value.length > 0) {
        attrs.push({ name: attr.name, value: attr.value });
      }
    }
    return attrs;
  }
  function elementIndexAmongSiblingsOfSameTag(element) {
  ...
  // ../../packages/selector-engine/src/generate.ts
  var BASE_SCORES = {
    id: 100,
    "data-attr": 90,
    aria: 80,
  ...
  function pushIfValid(candidates, root, target, selector, strategy, quality) {
  ...
  function generateIdCandidate(root, target, candidates) {
    const id = target.id;
    if (!id) return;
  ...
  function generateDataAttrCandidates(root, target, candidates) {
  ...
  function generateAriaCandidates(root, target, candidates) {
  ...
  function generateTagClassCandidate(root, target, candidates) {
  ...
  function shortAncestorSelector(element) {
    const id = element.id;
  ...
  function generateParentChildCandidate(root, target, candidates) {
    const parent = target.parentElement;
    if (!parent) return;
    const parentPart = shortAncestorSelector(parent);
    const stableClasses = stableClassTokens(target).slice(0, MAX_STABLE_CLASSES);
    const childPart = stableClasses.length > 0 ? `${tagOf(target)}${stableClasses.map((c) => `.${cssEscapeIdent(c)}`).join("")}` : tagOf(target);
  ...
  function generateNthOfTypeCandidate(root, target, candidates) {
    const parts = [];
    let current = target;
    let depth = 0;
  ...
  function generateSelectorCandidates(target, root = document) {
    const candidates = [];
  ...
  // ../../packages/selector-engine/src/pick.ts
  function pickBestCandidate(candidates, mode) {
  ...
  // ../../packages/selector-engine/src/navigate.ts
  var SelectionNavigator = class {
    current;
    history;
    constructor(initial) {
    ...
    previouslyVisitedChildOfCurrent() {
  ...
  var FAILURE_STATUS_CODES = STATUS_CODES.filter(
    (code) => code !== "SUCCESS"
  );
  ...
  var normalizationConfigSchema = external_exports.object({
    extractFirstNumber: external_exports.boolean().default(false),
    parsePrice: external_exports.boolean().default(false),
    regexPattern: external_exports.string().max(MAX_REGEX_PATTERN_LENGTH).optional(),
    regexFlags: external_exports.string().max(4).regex(/^[gimsuy]*$/).optional(),
  ...
  var rssTokenStatusSchema = external_exports.enum(["active", "revoked"]);
  var feedSchema = external_exports.object({
    id: external_exports.string(),
    name: external_exports.string().min(1).max(200),
    slug: external_exports.string().min(1).max(200).regex(/^[a-z0-9-]+$/),
    kind: feedKindSchema,
    rssTokenPrefix: external_exports.string().nullable(),
    rssTokenIssuedAt: external_exports.string().nullable(),
    rssTokenLastUsedAt: external_exports.string().nullable(),
    rssTokenStatus: rssTokenStatusSchema.nullable(),
    enabled: external_exports.boolean(),
    createdAt: external_exports.string(),
    updatedAt: external_exports.string()
  });
  var createFeedRequestSchema = external_exports.object({
    name: external_exports.string().min(1).max(200),
    slug: external_exports.string().min(1).max(200).regex(/^[a-z0-9-]+$/),
    kind: feedKindSchema.default("content"),
    enabled: external_exports.boolean().default(true)
  });
  var updateFeedRequestSchema = external_exports.object({
    name: external_exports.string().min(1).max(200).optional(),
    enabled: external_exports.boolean().optional()
  });
  var feedWithPlaintextTokenSchema = feedSchema.extend({
    rssToken: external_exports.string(),
    rssUrl: external_exports.string()
  });

  // ../../packages/shared/src/schemas/selection.ts
  var selectorTypeSchema = external_exports.enum(["css", "document"]);
  var extractionModeSchema = external_exports.enum(["text", "html", "attribute", "link", "image", "list"]);
  var matchModeSchema = external_exports.enum(["normalized", "case_insensitive"]);
  var selectorCandidateSchema = external_exports.object({
    selector: external_exports.string().min(1).max(2e3),
    strategy: external_exports.enum(["id", "data-attr", "aria", "tag-class", "parent-child", "nth-of-type"]),
  ...
  var selectionSchema = external_exports.object({
    ...
    selectorCandidates: external_exports.array(selectorCandidateSchema).default([]),
    extractionMode: extractionModeSchema,
    attributeName: external_exports.string().max(200).nullable(),
  ...
  var selectionInputSchema = external_exports.object({
    ...
    selectorType: selectorTypeSchema,
    selector: external_exports.string().max(2e3),
    selectorCandidates: external_exports.array(selectorCandidateSchema).max(20).default([]),
    extractionMode: extractionModeSchema,
    attributeName: external_exports.string().max(200).nullable().default(null),
  ...
  function hasAllowedProtocol(rawUrl) {
    try {
  ...
  var monitorWithSelectionsSchema = monitorSchema.extend({
    selections: external_exports.array(selectionSchema)
  });
  var createMonitorRequestSchema = external_exports.object({
    ...
    feedId: external_exports.string().min(1).optional(),
    name: external_exports.string().min(1).max(200),
  ...
  var updateMonitorRequestSchema = external_exports.object({
    feedId: external_exports.string().min(1).optional(),
    name: external_exports.string().min(1).max(200).optional(),
  ...
  // ../../packages/shared/src/schemas/monitor-state.ts
  var monitorStatusSchema = external_exports.enum(MONITOR_STATUSES);
  var extractedSelectionValueSchema = external_exports.object({
  ...
  var monitorStateSchema = external_exports.object({
    ...
    currentValue: external_exports.array(extractedSelectionValueSchema).nullable(),
    currentHash: external_exports.string().nullable(),
    lastCheckedAt: external_exports.string().nullable(),
    lastSuccessAt: external_exports.string().nullable(),
    lastChangedAt: external_exports.string().nullable(),
    consecutiveFailures: external_exports.number().int().nonnegative(),
    lastErrorCode: external_exports.string().nullable(),
    lastErrorMessage: external_exports.string().nullable(),
    updatedAt: external_exports.string()
  });

  // ../../packages/shared/src/schemas/check.ts
  var statusCodeSchema = external_exports.enum(STATUS_CODES);
  var checkSchema = external_exports.object({
    ...
    resultHash: external_exports.string().nullable(),
    errorCode: external_exports.string().nullable(),
    errorMessage: external_exports.string().nullable()
  });

  // ../../packages/shared/src/schemas/change.ts
  var changeTypeSchema = external_exports.enum(CHANGE_TYPES);
  var changeSchema = external_exports.object({
    id: external_exports.string(),
    feedId: external_exports.string(),
    monitorId: external_exports.string().nullable(),
    detectedAt: external_exports.string(),
    changeType: changeTypeSchema,
    oldValue: external_exports.array(extractedSelectionValueSchema).nullable(),
    newValue: external_exports.array(extractedSelectionValueSchema).nullable(),
    changedSelectionIds: external_exports.array(external_exports.string()),
    changeFingerprint: external_exports.string(),
    guid: external_exports.string(),
    sourceUrl: external_exports.string().nullable(),
    published: external_exports.boolean()
  });

  // ../../packages/shared/src/schemas/system-state.ts
  var alertStatusSchema = external_exports.enum(["healthy", "stale"]);
  var systemStateSchema = external_exports.object({
    id: external_exports.number().int(),
    lastRunnerRunAt: external_exports.string().nullable(),
    lastRunnerSuccessAt: external_exports.string().nullable(),
    lastRunnerRunId: external_exports.string().nullable(),
    heartbeatThresholdSec: external_exports.number().int().positive(),
    alertStatus: alertStatusSchema,
    activeAlertChangeId: external_exports.string().nullable(),
    lastWatchdogCheckedAt: external_exports.string().nullable(),
    updatedAt: external_exports.string()
  });
  ...
  var sessionResponseSchema = external_exports.object({
    ...
    csrfToken: external_exports.string().nullable()
  });
  var runnerMonitorListResponseSchema = external_exports.object({
    monitors: external_exports.array(monitorWithSelectionsSchema)
  });
  var MAX_ERROR_MESSAGE_LENGTH = 2e3;
  var runnerResultRequestSchema = external_exports.object({
    monitorId: external_exports.string().min(1),
    runId: external_exports.string().min(1),
    startedAt: external_exports.string(),
    finishedAt: external_exports.string(),
    status: statusCodeSchema,
    durationMs: external_exports.number().int().nonnegative(),
    httpStatus: external_exports.number().int().nullable().default(null),
    errorCode: external_exports.string().max(100).nullable().default(null),
    errorMessage: external_exports.string().max(MAX_ERROR_MESSAGE_LENGTH).nullable().default(null),
    ...
    values: external_exports.array(extractedSelectionValueSchema).default([])
  });
  var runnerResultResponseSchema = external_exports.object({
    monitorId: external_exports.string(),
    status: external_exports.enum(["baselined", "unchanged", "changed", "failed", "duplicate"]),
    changeId: external_exports.string().nullable()
  });
  var heartbeatEventSchema = external_exports.enum(["start", "complete"]);
  var heartbeatRequestSchema = external_exports.object({
  ...
  // src/content/overlay/shadow-root.ts
  var OVERLAY_HOST_ID = "web-monitor-overlay-root";
  var STYLES = `
  :host { all: initial; }
  .host-inner {
    position: fixed;
...
`;
  ...
  // src/content/selection-draft.ts
  function defaultExtractionModeFor(element) {
    const tag = element.tagName;
  ...
  function extractRaw(element, mode, attributeName) {
    switch (mode) {
      case "text":
      ...
      case "image":
        return resolveUrl(element.currentSrc || element.getAttribute("src"));
      case "list":
        return element.textContent ?? "";
    }
  }
  ...
  function createDraft(element, candidates, best, label, monitorMode = "single") {
    const extractionMode = monitorMode === "list" && best && best.matchCount > 1 ? "list" : defaultExtractionModeFor(element);
    return {
      id: nextSelectionId(),
      label,
      element,
  ...
  function createFullPageDraft(label) {
    return {
      id: nextSelectionId(),
      label,
  ...
  function renderPanel(panel, state, callbacks) {
    ...
    modeSelect.addEventListener(
      "change",
      () => callbacks.onMonitorModeChange(modeSelect.value)
    );
  ...
  function renderSelectionItem(selection, state, callbacks) {
    ...
    modeSelect.addEventListener(
      "change",
      () => callbacks.onExtractionModeChange(selection.id, modeSelect.value)
    );
