...
(() => {
  ...
  async function listDueLocalMonitors(config) {
    const res = await apiFetch(config, "/api/extension/monitors");
    const body = await res.json();
    return body.monitors;
  }
  async function submitExtensionResult(config, payload) {
  ...
  // src/background/local-check-runner.ts
  var MAX_ERROR_MESSAGE_LENGTH2 = 2e3;
  ...
  async function runExtraction(tabId, monitor) {
  ...
  async function runLocalCheck(config, monitor) {
    ...
    try {
      ...
      if (failure) {
        outcome = {
          ...
          errorMessage: failure.message.slice(0, MAX_ERROR_MESSAGE_LENGTH2),
        ...
        if ("statusCode" in extraction) {
          outcome = {
            ...
            errorMessage: extraction.message.slice(0, MAX_ERROR_MESSAGE_LENGTH2),
            durationMs: Date.now() - startedAt
          };
        } else {
          outcome = {
      ...
      outcome = {
        ...
        errorMessage: classified.message.slice(0, MAX_ERROR_MESSAGE_LENGTH2),
        durationMs: Date.now() - startedAt
      };
    } finally {
      if (tab?.id) {
  ...
  var localCheckInProgress = false;
  async function runDueLocalChecks(config) {
