...
- RSS 2.0 output per feed with stable GUIDs, `ETag`/`Last-Modified`/304 support, and rotatable, hashed (never plaintext-stored) delivery tokens. For a single-value change shown with the "old + new" display mode, the description carries only the lines that actually changed — five lines of context either side, the rest collapsed into a `… (N unchanged lines)` marker — with just the edited words marked inside them (struck-through old text, bold new text), rather than the whole before/after value. Word boundaries come from `Intl.Segmenter`, so this works for Japanese and other scripts that don't separate words with spaces.
