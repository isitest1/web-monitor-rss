...
決定日: 2026-09-14 / **状態: 完了・本番反映済み（2026-09-15）**
...
- 実装: `packages/shared/src/diff-text.ts`（Myers 差分 + トークナイザ + hunk 構築、依存追加なし）
- commit: `05c9ab1`（行単位 + 語単位、前後5行）、`a759978`（改行位置だけが変わった場合の対応）
- Worker version: `7c6481ca-b2bd-4cd7-bf17-34fdb6f03952`
- テスト: `diff-text.test.ts` を新規追加。`pnpm run ci` 全通過

計画段階では想定していなかったケースが、実データを流して判明したため追加対応した。
ページが文言を変えずに**改行位置だけを変えた**場合、行同士がほとんど対応しなくなり、
行単位の差分でも「旧値全体が削除・新値全体が追加」になってしまう。対応する行が
ほとんど無い場合は値全体を語単位で比較し直し、その比較では空白と改行の種類の違いを
差分として扱わないようにした（比較用の値が改行を空白へ畳むのと同じ理由）。

本番の2フィードにある変更イベント11件で測定した結果:

| | 反映前 | 反映後 |
|---|---|---|
| 「変更あり」として着色される文字の割合 | 99% | **15%** |
| フィード全体のサイズ | 145KB + 80KB | **86KB + 42KB** |
