...
> **このプロジェクトを開発・改造せず、ただ使いたいだけの場合**は、この文書ではなく [QUICKSTART.md](QUICKSTART.md)（英語、10分で完了）を参照してください。本書はDev Containerでの開発環境構築からハートビート検証まで含む、開発者向けの手順です。
...
- Feed作成時に平文のRSS URLが表示され、Feed管理画面を開けばいつでも同じURLを再確認できること。
- 表示されたトークンを含むRSS URLでフィードを取得できること。
- 「トークンを再発行」を実行すると、新しいRSS URLに置き換わり、旧トークンでの取得が拒否されること（再発行は明示的な操作でのみ行われ、自動では行われない）。
