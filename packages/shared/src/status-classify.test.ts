...
import { classifyException, classifyHttpStatus } from './status-classify.js';
...
describe('classifyException', () => {
  it('uses the explicit status code from a StatusCoded error', () => {
    const result = classifyException({ statusCode: 'SELECTOR_NOT_FOUND', message: 'no match' });
